...
DROP TABLE IF EXISTS JotFormRawData;
DROP TABLE IF EXISTS JotPlantContacts; -- removed in latest edit
DROP TABLE IF EXISTS JotFormIngredients;
DROP TABLE IF EXISTS JotFormProducts;
DROP TABLE IF EXISTS JotFormFiles;
DROP TABLE IF EXISTS JotFormFileLinks;
DROP TABLE IF EXISTS JotFormPlant;
DROP TABLE IF EXISTS JotFormContacts; -- removed in latest edit

-- Drop parent table last (referenced by foreign keys)
DROP TABLE IF EXISTS JotFormData; -- renamed
DROP TABLE IF EXISTS JotFormCompany;

-- This is the Company Application Table
CREATE TABLE JotFormCompany (
    JotFormId [int] IDENTITY(1,1) NOT NULL,
...
CREATE TABLE JotFormPlant (
    ...
    FOREIGN KEY (JotFormId) REFERENCES JotFormCompany(JotFormId),
    PRIMARY KEY CLUSTERED 
(
	[PlantId] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY]
...
CREATE TABLE JotFormProducts(
    JotFormProductId [int] IDENTITY(1,1) NOT NULL,
    JotPlantId [int]  NOT NULL,
    ...
    FOREIGN KEY (JotPlantId) REFERENCES JotFormPlant(PlantId),
    PRIMARY KEY CLUSTERED 
(
	[JotFormProductId] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY]
) ON [PRIMARY]
GO
...
CREATE TABLE JotFormIngredients(
    JotFormIngredientId [int] IDENTITY(1,1) NOT NULL,
    JotPlantId [int]  NOT NULL, 
    UKDID [nvarchar](50), 
    ...
    PRIMARY KEY CLUSTERED 
(
	[JotFormIngredientId] ASC
)WITH (PAD_INDEX = OFF, STATISTICS_NORECOMPUTE = OFF, IGNORE_DUP_KEY = OFF, ALLOW_ROW_LOCKS = ON, ALLOW_PAGE_LOCKS = ON, OPTIMIZE_FOR_SEQUENTIAL_KEY = OFF) ON [PRIMARY]
) ON [PRIMARY]
GO


CREATE TABLE JotFormRawData (
    JotFormRawDataId [int] IDENTITY(1,1) NOT NULL,  
    JotFormId [int]  NOT NULL,
