...
import logging
import time
from integration.vector_store.vector_store import add_plant_to_vectorstore, add_company_to_vectorstore, find_matching
...
def add_service(app, api, project_dir, swagger_host: str, PORT: str, method_decorators = []):
    ...
    def plantmatch():
        ...
        * Use standard Flask, here for non-database endpoints.
        ...
        plant = session.query(models.Facility).filter_by(facility_id=plant_id).first()
        if plant is None:
            return jsonify({ "error": f"Plant/Facility {plant_id} not found" }), 404
        ...
        match = {
           ...
           'postal': address.postal_code if address else None,
           'phone': None,
           "website": None
        }
        '''
    ...
    def match_company_vectorstore():
        ...
        * Use standard Flask, here for non-database endpoints.
        ...
        Test it with:
                ...
                http://devvm01.nyc.ou.org:5655/match_company_vectorstore?company_id=1
        """
        args = request.args
        company_id = args.get('company_id', default=3, type=int)
        
        company = session.query(models.JotFormCompany).filter_by(JotFormId=company_id).first()
        if company is None:
            return jsonify({ "error": f"JotFormCompany id: {company_id} not found" }), 404
        match = {
    ...
    def match_plant_vectorstore():
        ...
        * Use standard Flask, here for non-database endpoints.
        ...
        Test it with:
                ...
                http://devvm01.nyc.ou.org:5655/match_plant_vectorstore?company_id=1
        """
        args = request.args
        jotform_id = args.get('jotform_id', default=3, type=int)
        
        plant = session.query(models.JotFormPlant).filter_by(JotFormId=jotform_id).first()
        if plant is None:
            return jsonify({ "error": f"JotFormPlant {jotform_id} not found" }), 404
    ...
    def companymatch():
        ...
        * Use standard Flask, here for non-database endpoints.
        ...
        timers = {}
        global EXISTING_COMPANIES
        jotform_id = request.args.get('jotform_id', default=1, type=int)
        company = session.query(models.JotFormCompany).filter_by(JotFormId=jotform_id).first()
        if company is None:
            return jsonify({ "error": f"JotFormCompany {jotform_id} not found" }), 404
        ...
        match_start = time.time()
        result = match_company(match, EXISTING_COMPANIES, MatchConfig())
        match_end = time.time()
