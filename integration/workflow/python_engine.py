...
class PythonScriptEngine(object):
    ...
    def evaluate(self, task, expression, external_context=None) -> any:
        """
    ...
    def call_service(self, task, **kwargs) -> any:
    ...
    def create_task_exec_exception(self, task, script, err):
        ...
        if len(err.args) > 0:
            detail += f":{err.args[0]}"
        return detail #WorkflowTaskException(detail, task=task, exception=err, line_number=line_number, error_line=error_line)
