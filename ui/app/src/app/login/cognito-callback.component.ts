...
export class CognitoCallbackComponent implements OnInit {
  ...
  private handleAuthenticationSuccess(params: any): void {
    ...
    //debugger; // 🚨 Inspect auth parameters from backend
    ...
    setTimeout(() => {
      console.log('🚀 Navigating to /main with Cognito authentication...');
      console.log('🔍 Final auth check before navigation:', this.cognitoAuth.isLoggedIn());
      this.cognitoAuth.navigateToMain();
    }, 2000);
  }

  private handleAuthCode(code: string, state: string): void {
    console.log('Processing authorization code...'); // 🔍 SET BREAKPOINT HERE
    //debugger; // 🚨 Automatic breakpoint - will pause execution
  ...
  private processExistingSession(): void {
    ...
    this.cognitoAuth.processAuthCallback().subscribe({
      next: (response) => {
        ...
        //debugger; // 🚨 Automatic breakpoint - inspect response object
        
        // Ensure the token is stored and Authorization Bearer header will be set
        if (response.access_token) {
          console.log('Access token received and stored for Authorization Bearer header');
          //debugger; // 🚨 Inspect the access_token here
        }
